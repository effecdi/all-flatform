...
const STEPS: StepConfig[] = [
  ...
  },
  {
    key: "businessStage",
    emoji: "🚀",
    question: "창업 단계는 어디에 해당하나요?",
    subtitle: "현재 사업의 성장 단계를 선택해주세요",
    required: true,
    autoAdvance: true,
  },
  {
  ...
  },
  {
    ...
    subtitle: "주요 사업장이 위치한 지역을 선택해주세요",
    required: true,
    autoAdvance: true,
  },
  {
  ...
  },
  {
    ...
    emoji: "✍️",
    question: "마지막으로, 사업을 소개해주세요",
    subtitle: "AI가 더 정확한 추천을 드릴 수 있어요",
    required: false,
    autoAdvance: false,
  },
];
...
function ProgressBar({ current, total }: { current: number; total: number }) {
  ...
  return (
    <div className="fixed top-14 left-0 right-0 z-40 h-1 bg-border/30">
      <motion.div
...
function SelectionCard({
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      className={`
        relative text-left p-6 rounded-2xl border transition-colors
        backdrop-blur-sm cursor-pointer
        ${selected
          ? "bg-primary/10 border-primary shadow-lg shadow-primary/10 ring-2 ring-primary/30"
          : "bg-white/60 border-border/50 hover:border-primary/30 hover:bg-white/80"
        }
      `}
    ...
    <motion.button
      ...
      className={`
        ...
        ${selected
          ...
          : "bg-white/60 backdrop-blur-sm border-border/50 hover:border-primary/30 hover:bg-white/80 text-foreground"
        }
      `}
...
export default function OnboardingPage() {
  ...
  const canProceed = useCallback(() => {
    ...
    switch (step.key) {
      ...
      case "businessStage":
        return form.businessStage !== "";
      case "industrySector":
        return form.industrySector !== "";
      case "region":
        return form.region !== "";
      default:
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      // Enter to go next (Ctrl/Cmd+Enter for textarea step)
      if (e.key === "Enter") {
        ...
        return (
          <div className="max-w-md mx-auto">
            <Input
              ...
              className="h-14 rounded-xl bg-white/60 backdrop-blur-sm text-lg text-center border-border/50 focus:border-primary"
            />
          </div>
        );
          ...
          <div className="grid grid-cols-2 gap-4 max-w-lg mx-auto">
            {Object.entries(BUSINESS_STAGE_LABELS).map(([key, label]) => (
              <SelectionCard
                ...
                description={STAGE_META[key]?.desc || ""}
                selected={form.businessStage === key}
                onClick={() => {
                  update("businessStage", key);
                  triggerAutoAdvance();
                }}
              />
            ))}
          ...
          <div className="flex flex-wrap gap-3 justify-center max-w-lg mx-auto">
            {Object.entries(INDUSTRY_SECTOR_LABELS).map(([key, label], i) => (
              <PillButton
                key={key}
                label={label}
                selected={form.industrySector === key}
                delay={i}
                onClick={() => {
                  update("industrySector", key);
                  triggerAutoAdvance();
                }}
              />
            ))}
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                {REVENUE_RANGES.map((r, i) => (
                  <PillButton
                    ...
                    delay={i}
                    onClick={() => update("annualRevenue", r)}
                  />
                ))}
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                {FUNDING_RANGES.map((r, i) => (
                  <PillButton
                    key={r}
                    label={r}
                    selected={form.desiredFunding === r}
                    delay={i}
                    onClick={() => update("desiredFunding", r)}
                  />
                ))}
              </div>
            </div>
        ...
        return (
          <div className="max-w-md mx-auto">
            <textarea
              ...
              className="w-full rounded-xl bg-white/60 backdrop-blur-sm border border-border/50 focus:border-primary px-4 py-4 text-base min-h-[140px] resize-y focus:outline-none focus:ring-2 focus:ring-primary/20"
            />
            <p className="text-xs text-muted-foreground mt-2 text-center">
              Ctrl+Enter 또는 Cmd+Enter로 완료
            </p>
          </div>
        );
  ...
  if (completed) {
    return (
      <div className="min-h-[calc(100vh-3.5rem)] bg-gradient-to-br from-blue-50/50 via-background to-indigo-50/30">
        <CompletionScreen />
      </div>
    );
  }
          ...
          <div className="flex gap-2">
            {!stepConfig.required && (
              <Button
                ...
                disabled={isLastStep && saveProfile.isPending}
              >
                건너뛰기
              </Button>
            )}
            ...
            {isLastStep ? (
              <Button
                ...
                disabled={saveProfile.isPending || !canProceed()}
