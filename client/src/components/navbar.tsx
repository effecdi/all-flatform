...
export function Navbar() {
  ...
  return (
    <nav ref={menuRef} className="fixed top-0 left-0 right-0 z-50 bg-card/80 backdrop-blur-xl border-b border-border/40 shadow-glass">
      ...
      >
        <div className="bg-card/95 backdrop-blur-xl border-t border-border/40 px-5 py-4 space-y-1">
          {NAV_ITEMS.map((item) => {
            const Icon = item.icon;
          ...
          <div className="border-t border-border/40 pt-2 mt-2">
            ...
            {user && (
              <button
                onClick={handleLogout}
                className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-red-600 dark:text-red-400 hover:bg-red-500/10 transition-all duration-200 w-full text-left"
              >
